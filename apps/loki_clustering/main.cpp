...
{
    std::cout << "loki_clustering " << loki::VERSION_STRING << "\n";
}

static void printHelp()
{
    std::cout
        << "loki_clustering " << loki::VERSION_STRING
        << " -- k-means and DBSCAN clustering for time series\n"
        << "\nUsage:\n"
        << "  loki_clustering.exe <config.json> [options]\n"
        << "\nOptions:\n"
...
struct CliArgs {
    ...
    std::filesystem::path configPath {"config/clustering.json"};
};
...
static CliArgs parseArgs(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "--help"    || arg == "-h") { args.showHelp    = true; }
            ...
            std::cerr << "[loki_clustering] Unknown option: " << arg
                      << "  (use --help)\n";
        }
    }
    return args;
...
int main(int argc, char* argv[])
{
    ...
    try {
        loki::Logger::initDefault(cfg.logDir, "loki_clustering", cfg.output.logLevel);
    } catch (const loki::LOKIException& ex) {
        std::cerr << "[LOKI ERROR] Cannot init logger: " << ex.what() << "\n";
        return EXIT_FAILURE;
    }

    LOKI_INFO("loki_clustering " + std::string(loki::VERSION_STRING) + " started.");
    ...
    loki::clustering::ClusteringAnalyzer analyzer{cfg};
    ...
    for (const auto& r : loadResults) {
        ...
        for (const loki::TimeSeries& ts : r.series) {
            try {
                ...
                LOKI_ERROR("Clustering failed for '"
                           + ts.metadata().componentName + "': " + ex.what());
            }
        }
    }

    LOKI_INFO("loki_clustering finished successfully.");
    return EXIT_SUCCESS;
}
