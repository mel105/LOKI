...
{
    std::cout << "loki_spectral " << loki::VERSION_STRING << "\n";
}

static void printHelp()
{
    std::cout
        << "loki_spectral " << loki::VERSION_STRING
        << " -- Spectral analysis of time series (FFT / Lomb-Scargle)\n"
        << "\nUsage:\n"
        << "  loki_spectral.exe <config.json> [options]\n"
        << "\nOptions:\n"
...
struct CliArgs {
    ...
    std::filesystem::path configPath {"config/spectral.json"};
};
...
static CliArgs parseArgs(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "--help"    || arg == "-h") { args.showHelp    = true; }
            ...
            std::cerr << "[loki_spectral] Unknown option: " << arg
                      << "  (use --help)\n";
        }
    }
    return args;
...
int main(int argc, char* argv[])
{
    ...
    try {
        loki::Logger::initDefault(cfg.logDir, "loki_spectral", cfg.output.logLevel);
    } catch (const loki::LOKIException& ex) {
        std::cerr << "[LOKI ERROR] Cannot init logger: " << ex.what() << "\n";
        return EXIT_FAILURE;
    }

    LOKI_INFO("loki_spectral " + std::string(loki::VERSION_STRING) + " started.");
    ...
    // Run spectral analysis for every series in every loaded file.
    loki::spectral::SpectralAnalyzer analyzer{cfg};
    ...
    for (const auto& r : loadResults) {
        ...
        for (const loki::TimeSeries& ts : r.series) {
            try {
                ...
                LOKI_ERROR("Spectral analysis failed for '"
    ...
    LOKI_INFO("loki_spectral finished successfully.");
    return EXIT_SUCCESS;
}
