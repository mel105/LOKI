...
{
    std::cout << "loki_kalman " << loki::VERSION_STRING << "\n";
}

static void printHelp()
{
    std::cout
        << "loki_kalman " << loki::VERSION_STRING
        << " -- Kalman filter, RTS smoother, and EM noise estimation\n"
        << "\nUsage:\n"
        << "  loki_kalman.exe <config.json> [options]\n"
        << "\nOptions:\n"
...
struct CliArgs {
    ...
    std::filesystem::path configPath {"config/kalman.json"};
};
...
static CliArgs parseArgs(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "--help"    || arg == "-h") { args.showHelp    = true; }
            ...
            std::cerr << "[loki_kalman] Unknown option: " << arg
                      << "  (use --help)\n";
        }
    }
    return args;
...
int main(int argc, char* argv[])
{
    ...
    try {
        loki::Logger::initDefault(cfg.logDir, "loki_kalman", cfg.output.logLevel);
    } catch (const loki::LOKIException& ex) {
        std::cerr << "[LOKI ERROR] Cannot init logger: " << ex.what() << "\n";
        return EXIT_FAILURE;
    }

    LOKI_INFO("loki_kalman " + std::string(loki::VERSION_STRING) + " started.");
    ...
    // Run Kalman analysis for every series in every loaded file.
    loki::kalman::KalmanAnalyzer analyzer{cfg};
    ...
    for (const auto& r : loadResults) {
        ...
        for (const loki::TimeSeries& ts : r.series) {
            try {
                ...
                LOKI_ERROR("Kalman analysis failed for '"
    ...
    LOKI_INFO("loki_kalman finished successfully.");
    return EXIT_SUCCESS;
}
