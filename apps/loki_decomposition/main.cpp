...
{
    std::cout << "loki_decomposition " << loki::VERSION_STRING << "\n";
}

static void printHelp()
{
    std::cout
        << "loki_decomposition " << loki::VERSION_STRING
        << " -- Classical and STL time series decomposition\n"
        << "\nUsage:\n"
        << "  loki_decomposition.exe <config.json> [options]\n"
        << "\nOptions:\n"
...
struct CliArgs {
    ...
    std::filesystem::path configPath {"config/decomposition.json"};
};
...
static CliArgs parseArgs(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else { std::cerr << "[loki_decomposition] Unknown option: " << arg << "  (use --help)\n"; }
    }
    return args;
}
...
int main(int argc, char* argv[])
{
    ...
    try { loki::Logger::initDefault(cfg.logDir, "loki_decomposition", cfg.output.logLevel); }
    catch (const loki::LOKIException& ex) {
        std::cerr << "[LOKI ERROR] Cannot init logger: " << ex.what() << "\n"; return EXIT_FAILURE;
    }

    LOKI_INFO("loki_decomposition " + std::string(loki::VERSION_STRING) + " started.");
    ...
    // Load data via DataManager (same pattern as all other apps).
    ...
    // Run decomposition for every series in every loaded file.
    loki::DecompositionAnalyzer analyzer{cfg};
    ...
    for (const auto& r : loadResults) {
        ...
        for (const loki::TimeSeries& ts : r.series) {
            try {
                ...
                LOKI_ERROR("Decomposition failed for '"
    ...
    LOKI_INFO("loki_decomposition finished successfully.");
    return EXIT_SUCCESS;
}