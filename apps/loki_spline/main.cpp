...
{
    std::cout << "loki_spline " << loki::VERSION_STRING << "\n";
}

static void printHelp()
{
    std::cout
        << "loki_spline " << loki::VERSION_STRING
        << " -- B-spline approximation for time series\n"
        << "\nUsage:\n"
        << "  loki_spline.exe <config.json> [options]\n"
        << "\nOptions:\n"
        << "  --help     Show this message and exit.\n"
        << "  --version  Show version string and exit.\n"
        << "\nMethods (spline.method in config):\n"
        << "  bspline  -- B-spline LSQ approximation (implemented).\n"
        << "  nurbs    -- NURBS (placeholder, not yet implemented).\n"
        << "\nFit modes (spline.bspline.fit_mode in config):\n"
        << "  approximation       -- LSQ fit with nCtrl < nObs (default).\n"
        << "  exact_interpolation -- Passes through all data points (nCtrl = nObs).\n"
        << "                         Restricted to nObs <= exact_interpolation_max_n.\n";
}

struct CliArgs {
    bool                  showHelp   {false};
    bool                  showVersion{false};
    std::filesystem::path configPath {"config/spline.json"};
};
...
static CliArgs parseArgs(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (arg == "--help"    || arg == "-h") { args.showHelp    = true; }
            ...
            std::cerr << "[loki_spline] Unknown option: " << arg
                      << "  (use --help)\n";
        }
    }
    return args;
...
int main(int argc, char* argv[])
{
    ...
    try {
        loki::Logger::initDefault(cfg.logDir, "loki_spline", cfg.output.logLevel);
    } catch (const loki::LOKIException& ex) {
        std::cerr << "[LOKI ERROR] Cannot init logger: " << ex.what() << "\n";
        return EXIT_FAILURE;
    }

    LOKI_INFO("loki_spline " + std::string(loki::VERSION_STRING) + " started.");
    ...
    loki::spline::SplineAnalyzer analyzer{cfg};
    ...
    for (const auto& r : loadResults) {
        ...
        for (const loki::TimeSeries& ts : r.series) {
            try {
                ...
                LOKI_ERROR("Spline analysis failed for '"
                           + ts.metadata().componentName + "': " + ex.what());
            }
        }
    }

    LOKI_INFO("loki_spline finished successfully.");
    return EXIT_SUCCESS;
}
