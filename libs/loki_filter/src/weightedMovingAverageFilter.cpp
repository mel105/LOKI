...
WeightedMovingAverageFilter::WeightedMovingAverageFilter(Config cfg)
    : m_cfg{std::move(cfg)}
{
    if (m_cfg.weights.empty()) {
        throw ConfigException(
            "WeightedMovingAverageFilter: weights must not be empty.");
    }
}

FilterResult WeightedMovingAverageFilter::apply(const TimeSeries& series) const
{
    if (series.empty()) {
        throw DataException(
            "WeightedMovingAverageFilter::apply: input series is empty.");
    }
    ...
    std::vector<double> filtered =
        loki::stats::weightedMovingAverage(values, m_cfg.weights);
    ...
    const int    w   = static_cast<int>(m_cfg.weights.size());
    const int    n   = static_cast<int>(series.size());
    const double bw  = (n > 0) ? static_cast<double>(w) / static_cast<double>(n) : 0.0;
    FilterResult res{std::move(filteredSeries), std::move(residualSeries), name()};
    res.effectiveWindow    = w;
    res.effectiveBandwidth = bw;
    return res;
}

std::string WeightedMovingAverageFilter::name() const
{
