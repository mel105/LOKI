...
void GnssCsvExport::exportAll(const GnssResult& result, const ObsFile& obs) const
{
    ...
    if (result.hasPpp) {
        exportPppEpochs(result);
        exportPppTropo(result.ppp, obs);
    }
}
...
void GnssCsvExport::exportSppEpochs(const GnssResult& result) const
{
    ...
    for (const auto& r : result.spp) {
        ...
        f << std::fixed << std::setprecision(9) << r.time.toTimeStamp().mjd() << ";"
          << std::setprecision(4)
          << r.x << ";" << r.y << ";" << r.z << ";"
          << std::setprecision(9) << gp.lat << ";" << gp.lon << ";"
          << std::setprecision(4) << gp.h << ";"
          << r.clkBiasM << ";" << r.clkBiasM / C * 1.0e6 << ";"
          << std::setprecision(3) << r.pdop << ";"
          << r.nSats << ";"
          << (r.converged ? 1 : 0) << ";"
          << std::setprecision(4) << meanRes << ";" << rmsRes << "\n";
    }
...
// =============================================================================
//  exportTropo  (SPP -- Saastamoinen standard atmosphere, constant ZHD/ZWD)
// =============================================================================
                                  ...
                                  const ObsFile& obs) const
{
    ...
    f << "# ZTD [mm]    : " << ztd  * 1000.0 << "\n";
    f << "# NOTE        : ZHD and ZWD are constant (std atm); use PPP+VMF3 for epoch-varying ZWD\n";
    f << "#\n";
    f << "# epoch_utc;doy;sow;zhd_mm;zwet_mm;ztd_mm;mean_elev_deg;slant_delay_m\n";

    for (const auto& r : spp) {
        if (!r.valid) continue;
        double sumEl = 0.0; int nEl = 0;
        for (const auto& us : r.usedSats) { sumEl += us.elevation; ++nEl; }
        const double meanElDeg = (nEl > 0) ? sumEl / nEl : 15.0;
        const double sinEl     = std::max(std::sin(meanElDeg * (std::numbers::pi / 180.0)), 0.05);
        ...
        f << epoch.str() << ";"
          ...
          << std::setprecision(3)
          << zhd * 1000.0 << ";" << zwet * 1000.0 << ";" << ztd * 1000.0 << ";"
...
// =============================================================================
//  exportPppEpochs
//  One row per valid epoch: mjd; x; y; z; lat; lon; h; clk_m; ztd_wet_m;
//  ztd_total_m; n_sats; converged; code_rms_m; phase_rms_m
// =============================================================================

void GnssCsvExport::exportPppEpochs(const GnssResult& result) const
{
    fs::create_directories(m_cfg.csvDir);
    const std::string path = outPath("ppp_epochs");
    std::ofstream f(path);
    if (!f.is_open())
        throw IoException("GnssCsvExport: cannot open " + path);
    ...
    f << "mjd;x_m;y_m;z_m;lat_deg;lon_deg;h_m;"
         "clk_m;clk_us;ztd_wet_m;ztd_total_m;"
         "n_sats;converged;code_rms_m;phase_rms_m\n";

    for (const auto& r : result.ppp) {
        if (!r.valid) continue;
...
// =============================================================================
//  exportPppTropo
//  Per-epoch: epoch; doy; sow; ztd_wet_m (filter); ztd_total_m; n_sats; converged
// =============================================================================

void GnssCsvExport::exportPppTropo(const std::vector<PppResult>& ppp,
                                    const ObsFile& /*obs*/) const
{
    if (ppp.empty()) return;
    fs::create_directories(m_cfg.csvDir);
    const std::string path = outPath("ppp_tropo");
    std::ofstream f(path);
    if (!f.is_open())
        throw IoException("GnssCsvExport: cannot open " + path);

    f << "# LOKI GNSS -- PPP TROPOSPHERE (Kalman-estimated ZTD wet)\n";
    f << "# ZTD_wet : estimated by Kalman filter (random-walk parameter)\n";
    f << "# ZTD_tot : ZHD (Saastamoinen std atm, fixed) + ZTD_wet (estimated)\n";
    f << "#\n";
    f << "# epoch_utc;doy;sow;ztd_wet_m;ztd_total_m;n_sats;converged\n";
