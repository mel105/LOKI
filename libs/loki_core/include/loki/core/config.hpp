...
struct PlotConfig {
    ...
    // -- Spatial analysis (kriging) pipeline plots ---------------------------------
...
/**
 * @brief Input configuration for the loki_spatial loader.
 *
 * Replaces the generic InputConfig for spatial data files.
 *
 * commentPrefix: lines starting with this string are treated as comments.
 *   Accepted values: "#", "%", "!", "//", ";", "*"
 *
 * delimiter: single character separating columns.
 *   Use ' ' (space) to split on any whitespace sequence (most common).
 *
 * xColumn / yColumn: 0-based column indices for the X and Y coordinates.
 *
 * valueColumns: 0-based indices of columns to interpolate.
 *   Empty vector = all columns except xColumn and yColumn.
 *
 * noDataValue / noDataTolerance: observations with
 *   |value - noDataValue| <= noDataTolerance are skipped for that variable.
 *
 * coordinateUnit: informational only ("m", "km", "deg", "rad", "").
 *   No transformation is applied. For LLA in small regions, treat as
 *   Cartesian. For geodetic accuracy, project first with loki_geodesy.
 */
struct SpatialInputConfig {
    std::string          file             = "";
    char                 delimiter        = ' ';
