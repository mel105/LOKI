 ...
 * Usage example:
 * @code
 *   AppConfig cfg = ConfigLoader::load("config/qc.json");
 *   Logger::initDefault(cfg.logDir, "loki_qc", cfg.output.logLevel);
 * @endcode
 */
class ConfigLoader {
public:
...
private:
    ...
    static SimulateConfig        _parseSimulate         (const nlohmann::json& j);
