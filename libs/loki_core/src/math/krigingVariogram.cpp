    ...
    const KrigingVariogramConfig& cfg)
{
    ...
    if (maxLag <= 0.0) {
        const double minMjd = *std::min_element(mjds.begin(), mjds.end());
        const double maxMjd = *std::max_element(mjds.begin(), mjds.end());
        maxLag = (maxMjd - minMjd) * 0.5;
    }
    ...
    for (int k = 0; k < nBins; ++k) {
        ...
        if (cnt < 2) continue;
        VariogramPoint pt;
        pt.lag   = (static_cast<double>(k) + 0.5) * binSize;
                         ...
                         double& nugget0, double& sill0, double& range0)
{
    ...
    if (sill0 <= 0.0 || range0 <= 0.0) {
        ...
        if (range0 <= 0.0) range0 = maxLag * 0.5;
        if (nugget0 <= 0.0 && empirical.size() >= 2) {
            ...
            const double slope = (g2 - g1) / (h2 - h1 + 1.0e-12);
            nugget0 = std::max(0.0, g1 - slope * h1);
        }
    }
                    ...
                    double nugget, double sill, double range)
{
    ...
    if (nugget < 0.0 || range <= 0.0) return BIG;
    if (model == "power") {
        if (sill <= 0.0 || range >= 2.0) return BIG;
    } else {
                                ...
                                const KrigingVariogramConfig&      cfg)
{
    ...
    if (cfg.model != "spherical"   && cfg.model != "exponential" &&
        cfg.model != "gaussian"    && cfg.model != "power"       &&
        cfg.model != "nugget") {
        throw AlgorithmException(
    ...
    if (cfg.model == "nugget") {
        result.nugget = (cfg.nugget > 0.0) ? cfg.nugget : empirical.back().gamma;
    ...
    double nugget0 = 0.0, sill0 = 0.0, range0 = 0.0;
    ...
    const auto nmResult = nelderMead(objective, {nugget0, sill0, range0}, 2000, 1.0e-9);
