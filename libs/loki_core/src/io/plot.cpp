...
{
    if (ts.empty()) {
        throw DataException("Plot::timeSeries(): time series is empty.");
    }
...
void Plot::comparison(const TimeSeries& analysed,
                      ...
                      const std::string& title)
{
    ...
    // matched by index (both series are assumed to have the same sampling).
    const std::size_t n = std::min(analysed.size(), reference.size());
    ...
    try {
        ...
        gp("set bmargin 3");
        gp("set xlabel 'Time'");
        gp("set ylabel 'Difference'");
        gp("set key off");
        // Col 4 is computed inline in gnuplot as ($2-$3)
...
{
    if (ts.empty()) {
        throw DataException("Plot::acf(): time series is empty.");
    }
    ...
    for (const auto& obs : ts) {
        if (isValid(obs)) { vals.push_back(obs.value); }
    }
...
{
    if (ts.empty()) {
        throw DataException("Plot::histogram(): time series is empty.");
    }
...
{
    if (ts.empty()) {
        throw DataException("Plot::qqPlot(): time series is empty.");
    }
...
{
    if (ts.empty()) {
        throw DataException("Plot::boxplot(): time series is empty.");
    }
...
{
    if (values.empty()) {
        throw DataException("Plot::timeSeries(): values vector is empty.");
    }
    ...
    const std::string stem = title;
    const auto dataFile    = writeTempData(".tmp_" + stem, xy);
                      ...
                      const std::string& title)
{
    ...
    if (analysed.size() != reference.size()) {
        throw DataException("Plot::comparison(): analysed and reference vectors must be the same size.");
    }
...
void Plot::acf(const std::vector<double>& values, int maxLag, const std::string& title)
{
    ...
    if (static_cast<int>(n) < maxLag + 2) {
        throw DataException(
            "Plot::acf(): series too short for maxLag=" + std::to_string(maxLag) +
            " (need at least " + std::to_string(maxLag + 2) + " valid observations, got " +
            std::to_string(n) + ").");
    }

    const auto acfData  = computeAcf(valid, maxLag);
    ...
    try {
        ...
        // Confidence band as horizontal lines
...
void Plot::histogram(const std::vector<double>& values, int bins, const std::string& title)
{
    const auto valid = validValues(values);
    if (valid.empty()) {
        throw DataException("Plot::histogram(): no valid values in vector.");
    }
    ...
    variance /= static_cast<double>(valid.size());
    const double sigma = std::sqrt(variance);

    // Write raw values (gnuplot will bin them)
    ...
    const std::string binsStr  = std::to_string(bins);
    ...
    try {
        ...
        // Normal distribution overlay (scaled to match histogram counts)
        // N(x) = (n_samples * width) * gaussian(x; mean, sigma)
...
{
    const auto qqData   = computeQQ(values);   // throws if too few values
    const auto dataFile = writeTempData(".tmp_" + title, qqData);

    // Reference line: min and max of theoretical quantiles
    ...
    try {
        ...
        const std::string xMinStr = std::to_string(xMin);
        const std::string xMaxStr = std::to_string(xMax);

        gp("plot '" + dataFile.string() + "' u 1:2 t 'Data' w p pt 7 ps 0.6 lc rgb '#2166ac', "
           "x t 'Normal line' w l lc rgb '#d73027' lw 2");
    } catch (...) {
        removeTempFile(dataFile);
        throw;
    }
...
void Plot::boxplot(const std::vector<double>& values, const std::string& title)
{
    const auto valid = validValues(values);
    if (valid.empty()) {
        throw DataException("Plot::boxplot(): no valid values in vector.");
    }

    // Compute descriptive statistics for annotation
    const std::size_t n = valid.size();
    auto sorted = valid;
    ...
    // Write single-column data (gnuplot boxplot uses column 2 with x=0)
    std::vector<std::pair<double, double>> raw;
    raw.reserve(n);
    for (const double v : valid) { raw.emplace_back(0.0, v); }
    const auto dataFile = writeTempData(".tmp_" + title, raw);
    ...
    try {
        ...
        // Stats annotation in top-left corner
        gp("set label 1 '" + label + "' at graph 0.05, graph 0.95 left top font ',10'");

        gp("plot '" + dataFile.string() + "' u (0):2 notitle");
    } catch (...) {
        removeTempFile(dataFile);
        throw;
    }
...
{
    // PlotConfig::timeFormat overrides InputConfig::timeFormat when set.
    // An empty string means "inherit from input".
    if (!m_config.plots.timeFormat.empty()) {
        // Mirror the string-to-enum logic from ConfigLoader.
    ...
    const std::vector<std::pair<double, double>>& data) const
{
    ...
    std::ofstream f(path);
    if (!f.is_open()) {
        throw IoException("Plot::writeTempData(): cannot open '" + path.string() + "' for writing.");
    }
    ...
    const std::vector<std::vector<double>>& columns) const
{
    ...
    for (const auto& col : columns) {
        if (col.size() != rows) {
            throw DataException(
                "Plot::writeTempDataMulti(): all columns must have the same number of rows.");
        }
    }
    ...
    if (!f.is_open()) {
        throw IoException(
            "Plot::writeTempDataMulti(): cannot open '" + path.string() + "' for writing.");
    }
...
void Plot::removeTempFile(const std::filesystem::path& path) noexcept
{
    ...
    // Errors are intentionally ignored — temp file cleanup is best-effort.
}

std::vector<std::pair<double, double>> Plot::toXY(const TimeSeries& ts) const
{
    if (ts.empty()) {
        throw DataException("Plot::toXY(): time series is empty.");
    }
    ...
    for (const auto& obs : ts) {
        ...
        switch (fmt) {
            ...
            case TimeFormat::GPS_WEEK_SOW:
                // Convert GPS time to MJD for display purposes.
                // Displaying raw GPS seconds or week+sow on an axis is not readable.
                x = obs.time.mjd();
                break;
            case TimeFormat::UTC:
                // For UTC we also use MJD numerically; gnuplot time axis
                // handles the formatting via set xdata time + timefmt.
                x = obs.time.mjd();
                break;
            case TimeFormat::INDEX:
            default:
                x = static_cast<double>(xy.size());
                break;
        }
...
{
    // Only UTC display needs gnuplot time axis setup.
    // For MJD, INDEX, GPS->MJD we use plain numeric axes.
    if (effectiveTimeFormat() == TimeFormat::UTC) {
        return "%s";  // Unix seconds — toXY() writes mjd, but gnuplot needs %s for unixTime
    }
    return {};
}
...
Plot::computeAcf(const std::vector<double>& values, int maxLag)
{
    ...
    if (var == 0.0) {
        throw AlgorithmException("Plot::computeAcf(): series has zero variance.");
    }
...
Plot::computeQQ(const std::vector<double>& values)
{
    ...
    if (valid.size() < 3) {
        throw DataException(
            "Plot::computeQQ(): need at least 3 valid observations, got " +
    ...
    // Approximate normal quantile using the Beasley-Springer-Moro algorithm.
    // For each order statistic i, the plotting position is (i - 0.375) / (n + 0.25).
    auto normalQuantile = [](double p) -> double {
        // Rational approximation (Abramowitz & Stegun 26.2.17).
        static constexpr double C0 =  2.515517;
        static constexpr double C1 =  0.802853;
        static constexpr double C2 =  0.010328;
        static constexpr double D1 =  1.432788;
        static constexpr double D2 =  0.189269;
        static constexpr double D3 =  0.001308;

        const bool upper = (p > 0.5);
        const double q = upper ? (1.0 - p) : p;
        const double t = std::sqrt(-2.0 * std::log(q));
        const double num = C0 + t * (C1 + t * C2);
        const double den = 1.0 + t * (D1 + t * (D2 + t * D3));
        const double z   = t - num / den;
        return upper ? z : -z;
    };
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        const double th = normalQuantile(p);
        result.emplace_back(th, sorted[i]);
    }
...
std::vector<double> Plot::validValues(const std::vector<double>& values)
{
    ...
    for (const double v : values) {
        if (v == v) { out.push_back(v); }  // NaN != NaN
    }
    return out;
}